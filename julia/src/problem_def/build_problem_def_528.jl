...
@load "../../jld2_files/problem_def_sample.jld2" problem_def_sample normals
cd(old_dir)

# how many components you will use
n_out = problem_def_sample.n_out
# how many differentiated versions of the original GP you will use
n_dif = problem_def_sample.n_dif
...
# best fit params of AIC 1 jones model from a subsection of SOAP 2.0 data saved in problem_def_sample.jld2 (with wrong size spots but whatever)
...
temp_prob_def = build_problem_definition(problem_def_sample.kernel, problem_def_sample.n_kern_hyper, build_problem_definition(n_dif, n_out, x_obs, x_obs_units, a0))
...
# end
amount_of_measurements = length(problem_def_sample.x_obs)
measurement_noise_mag = [mean(problem_def_sample.noise[((i - 1) * amount_of_measurements + 1):(i * amount_of_measurements)]) for i in 1:n_out]
measurement_noise = zeros(amount_of_total_samp_points)
for i in 1:n_out
...
problem_def_528 = build_problem_definition(problem_def_sample.kernel, problem_def_sample.n_kern_hyper, build_problem_definition(n_dif, n_out, x_obs, x_obs_units, y_obs, "Normalized RV + PCA scores", measurement_noise, a0))
