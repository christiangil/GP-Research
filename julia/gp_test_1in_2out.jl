...
# Differentiated Radial basis function kernel (aka squared exonential, ~gaussian)
...
function dRBF_kernel(hyperparameters, dif, dorder)
    ...
    if sum(dorder) > 0
        ...
        elseif sum(dorder) == 4
            value = value * (T1 ^ 4 - 6 * dif ^ 2 / (kernel_length ^ 6) + 3 / (kernel_length ^ 4))
            # if dorder==[2, 2]
        end
    end
    return value
...
function kernel(hyperparameters, x1, x2; dorder = [0,0])
    ...
    final = dRBF_kernel(append!([1],hyperparameters[1]), dif_tot, dorder)
...
x1_samp = linspace(0, domain, GP_sample_amount)
...
K_samp = Array{Any}(n_out, n_out, n_dif, n_dif)
for i in 1:n_out
    for j in 1:n_out
        for j in 1:n_dif
            for l in 1:n_dif
                K_samp[i,j,k,l] = covariance(x_samp, x_samp, append(collect(Iterators.flatten(a)), hyperparameters));
            end
        end
    end
end
