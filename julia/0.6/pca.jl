...
figure(figsize=(10,6))
fig = plot(collect(1:length(remaining_variance)), log10.(remaining_variance))
...
figure(figsize=(10,6))
fig = plot(collect(1:length(remaining_variance)), remaining_variance)
xlabel("Number of Principal Components")
...
#
#
# @time pca_eford_out = fit_pca_eford(obs, num_components=6);   # compute first few scores using Eric's iterative PCA routine
...
@time doppler_comp = calc_doppler_component_simple(lambda, obs)
...
@time genpca_out = fit_gen_pca_rv(obs, doppler_comp, num_components=6)
