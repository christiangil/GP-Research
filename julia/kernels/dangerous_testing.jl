...
function kernel_coder(symbolic_kernel_original::Basic, kernel_name::String)
    ...
    write(io, "function " * kernel_name * "_sym(hyperparameters::Array{Float64,1}, dif::Float64; dorder::Array{}=zeros(1))\n\n")
...
dpdt = Periodic_kernel_base([kernel_length, kernel_period], sqrt((t1 - t2) ^ 2))
kernel_name = "Periodic_kernel"
kernel_coder(dpdt, kernel_name)
include("kernels/" * kernel_name * ".jl")

init_plot()
fig = plot([i/20 for i in 1:100], [Periodic_kernel([2., 1], i/20) for i in 1:100])
fig = plot([i/20 for i in 1:100], [Periodic_kernel_sym([2., 1], i/20) for i in 1:100])
savefig("test.pdf")
