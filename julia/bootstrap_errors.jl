...
hdf5_filename = string(split(hdf5_loc,"/")[end])

fid = h5open(hdf5_loc, "r")
act = fid["active"]
@elapsed time_series_spectra = act[:, :]

# bootstrap_errors(time_series_spectra, hdf5_filename; boot_amount=1, save_filename="jld2_files/bootstrap" * file_str * ".jld2")

boot_amount = 10
for i in 1:5
    bootstrap_errors(time_series_spectra, hdf5_filename; boot_amount=boot_amount, save_filename="jld2_files/bootstrap" * file_str * ".jld2")
