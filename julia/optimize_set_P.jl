...
best_period = (sqrt(2)*6)u"d"

# draw over more periods?
original_ks = kep_signal(K_val * u"m/s", best_period, 2 * π * rand(rng), rand(rng) / 5, 2 * π * rand(rng), 0u"m/s")
add_kepler_to_Jones_problem_definition!(problem_definition, original_ks)
if star_choice == 0
    results_dir = "results/short/$n_out/$sample_size/$(kernel_name)/K_$(string(ustrip(original_ks.K)))_set_P/seed_$(seed)/"
end
if star_choice == 1
    results_dir = "results/long/$n_out/$sample_size/$(kernel_name)/K_$(string(ustrip(original_ks.K)))_set_P/seed_$(seed)/"
end
if star_choice == 2
    results_dir = "results/none/$n_out/$sample_size/$(kernel_name)/K_$(string(ustrip(original_ks.K)))_set_P/seed_$(seed)/"
end
try
...
function fit_kep_hold_P(Σ_obs::Cholesky{T,Matrix{T}} where T<:Real; ks=fit_kepler(problem_definition, Σ_obs, kep_signal_epicyclic(P=best_period)))  # 40x slower than just epicyclic fit. neither optimized including priors
...
begin
    ...
    global current_ks = fit_kep_hold_P(Σ_obs)
    ...
    result_change = Inf
    global num_iter = 0
    while result_change > 3e-4 && num_iter < 20
        global current_ks = fit_kep_hold_P(workspace.Σ_obs; ks=current_ks)
...
H2 = H2[1:end .!= n_hyper+2, 1:end .!= n_hyper+2]  # removing period
try
...
H3 = H3[1:end .!= n_hyper+2, 1:end .!= n_hyper+2]  # removing period
try
...
save_nlogLs(seed, [time1, time2, time4] ./ 3600, saved_likelihoods, append!(copy(fit1_total_hyperparameters), fit3_total_hyperparameters), original_ks, full_ks, results_dir)
