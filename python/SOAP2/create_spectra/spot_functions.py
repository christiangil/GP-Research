...
def msh2r_star(msh):
	return np.sqrt(msh * 2e-6 * np.pi)


def r_star2msh(r_star):
	return r_star**2 * 5e5 / np.pi
...
def start_times(spot_density, obs_n_years):
    ...
    N_spots = int(365 * spot_density * obs_n_years / PROT)
...
def spot_lifetime(sizes, types, Gf):
    ...
    lifetimes = np.multiply(time_mod, np.divide((sizes ** 2)/(2e-6 * np.pi), pillet_draw(types)) / 365)  # convert sizes to MSH, divide by decay rate, convert to years
